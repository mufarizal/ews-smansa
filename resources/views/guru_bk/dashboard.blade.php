...
@section('content')
    <div class="mx-auto max-w-7xl space-y-6">
                        ...
                        <ul class="mt-1.5 space-y-1">
                            @foreach ($kelasBelumGenerate as $gbk)
                                <li class="text-xs text-amber-700 flex items-center justify-between">
                                    <span><span class="font-medium">{{ $gbk->kelas->nama_kelas ?? '-' }}</span> — belum
                            ...
                            @foreach ($kelasStale as $gbk)
                                ...
                                <li class="text-xs text-amber-700 flex items-center justify-between">
                                    <span>
                                        <span class="font-medium">{{ $gbk->kelas->nama_kelas ?? '-' }}</span> — terakhir
                                        di-generate
        ...
        @if (isset($ringkasan['total']) && $ringkasan['total'] > 0)
            ...
            @push('scripts')
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const labels = @json($guruBkKelas->map(fn($g) => $g->kelas->nama_kelas ?? '-'));

                        const allData = [...@json($chartBinaan), ...@json($chartPerhatian), ...@json($chartAman)].filter(v => v > 0);
                                ...
                                @foreach ($siswaPreview as $item)
                                    <div class="flex items-center justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
                                        ...
                                        <div class="flex items-center gap-2 shrink-0">
                                            ...
                                            <span
                                                class="text-sm font-bold font-mono text-gray-700">{{ number_format($item->skor_akhir ?? 0, 2) }}</span>
                                            @include('partials.trend-indicator', [
