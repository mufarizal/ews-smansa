@extends('layouts.app')
@section('title', 'Kerjakan Tugas: ' . $tugas->judul)

@section('content')
<div class="mx-auto max-w-4xl">
    <div class="mb-6">
        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Tugas Online</p>
        <h1 class="mt-1.5 text-2xl font-bold text-gray-900">{{ $tugas->judul }}</h1>
        <p class="mt-1 text-sm text-gray-500">
            {{ $tugas->guruMapelKelas->mapel->nama ?? '-' }}
        </p>
    </div>

    <form method="POST" action="{{ route('siswa.tugas.submit', $tugas) }}" id="tugas-form">
        @csrf
                ...
                <article class="rounded-2xl border border-gray-200 bg-white p-6">
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 cursor-pointer">
                            <input type="radio" name="jawaban[{{ $soal->id }}][jawaban]" value="a" required
                        ...
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 cursor-pointer">
                            <input type="radio" name="jawaban[{{ $soal->id }}][jawaban]" value="b" required
                                class="h-4 w-4 border-gray-300 text-green-600 focus:ring-green-500">
                            <span class="text-sm"><span class="font-semibold text-blue-600">B.</span> {{ $soal->opsi_b }}</span>
                        </label>
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 cursor-pointer">
                            <input type="radio" name="jawaban[{{ $soal->id }}][jawaban]" value="c" required
                                class="h-4 w-4 border-gray-300 text-green-600 focus:ring-green-500">
                            <span class="text-sm"><span class="font-semibold text-amber-600">C.</span> {{ $soal->opsi_c }}</span>
                        </label>
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 cursor-pointer">
                            <input type="radio" name="jawaban[{{ $soal->id }}][jawaban]" value="d" required
                                class="h-4 w-4 border-gray-300 text-green-600 focus:ring-green-500">
                            <span class="text-sm"><span class="font-semibold text-rose-600">D.</span> {{ $soal->opsi_d }}</span>
                        </label>
                        <input type="hidden" name="jawaban[{{ $soal->id }}][soal_id]" value="{{ $soal->id }}">
                    </div>
                </article>
            @endforeach
        </div>
