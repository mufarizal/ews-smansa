...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="mt-2 text-sm text-gray-600">Perbarui data semester akademik</p>
    </div>
    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 bg-white">
        <form action="{{ route('kurikulum.semesters.update', $semester->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" id="nama" name="nama" value="{{ old('nama', $semester->nama) }}"
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="date" id="tanggal_mulai" name="tanggal_mulai"
                            value="{{ old('tanggal_mulai', date('Y-m-d', strtotime($semester->tanggal_mulai))) }}"
                            class="mt-2 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100"
                            required />
                        @error('tanggal_mulai')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date" id="tanggal_selesai" name="tanggal_selesai"
                            value="{{ old('tanggal_selesai', date('Y-m-d', strtotime($semester->tanggal_selesai))) }}"
                            class="mt-2 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100"
                            required />
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3"
                        class="mt-2 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100"
                        placeholder="Catatan tambahan...">{{ old('keterangan', $semester->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
