...
<div class="mx-auto max-w-2xl">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Tambah Materi Baru</h2>

        <form method="POST" action="{{ route('guru_mapel.bab.materi.store', $bab) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <textarea id="isi_materi" name="isi_materi" rows="5"
                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100">{{ old('isi_materi') }}</textarea>
                @error('isi_materi')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
