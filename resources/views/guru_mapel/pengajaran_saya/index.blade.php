...
@section('content')
    <div class="mx-auto max-w-7xl" x-data="pengajaranSaya()">
        ...
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Guru Mata Pelajaran</p>
                <h1 class="mt-1.5 text-2xl font-bold text-gray-900">Pengajaran Saya</h1>
                <p class="mt-1 text-sm text-gray-500">Mata pelajaran, kelas, daftar siswa, dan jadwal mengajar Anda.</p>
            </div>

            @if ($guru)
                <form method="GET" action="{{ route('guru_mapel.pengajaran-saya.index') }}"
                    class="flex items-end gap-2 shrink-0">
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Semester</label>
                        <select name="semester_id" onchange="this.form.submit()"
                            class="rounded-lg border border-gray-300 bg-white py-2 pl-3 pr-8 text-sm text-gray-700 shadow-sm focus:border-green-600 focus:ring-2 focus:ring-green-100">
                    ...
                    @if ($selectedSemesterId)
                        <a href="{{ route('guru_mapel.pengajaran-saya.index') }}"
        ...
        @else
            ...
            <div class="mb-6 rounded-xl border border-gray-200 bg-white p-5">
                <div class="flex items-center justify-between gap-4 mb-4">
                    <div class="flex items-center gap-3">
                        <div
                            class="flex h-10 w-10 items-center justify-center rounded-full bg-green-100 text-green-700 font-bold text-sm shrink-0">
                ...
                <div class="flex gap-3">
                    <div class="rounded-lg bg-gray-50 py-3 text-center flex-1 min-w-0">
                        <p class="text-lg font-bold text-gray-900">{{ $mapels->count() }}</p>
                        <p class="mt-0.5 text-gray-500 text-[11px] leading-[1.3]">Mapel<br>Diampu</p>
                    </div>
                    <div class="rounded-lg bg-gray-50 py-3 text-center flex-1 min-w-0">
                    ...
                    <div class="rounded-lg py-3 text-center {{ $jadwalHariIni->count() > 0 ? 'bg-green-50' : 'bg-gray-50' }} flex-1 min-w-0">
                        <p
                            class="text-lg font-bold {{ $jadwalHariIni->count() > 0 ? 'text-green-700' : 'text-gray-900' }}">
                            {{ $jadwalHariIni->count() }}
                        </p>
                        <p class="mt-0.5 {{ $jadwalHariIni->count() > 0 ? 'text-green-600' : 'text-gray-500' }} text-[11px] leading-[1.3]">Jadwal<br>Hari Ini</p>
                    </div>
                </div>
            </div>
            ...
            @if ($jadwalHariIni->count() > 0)
                <div class="mb-6 rounded-xl border border-green-200 bg-green-50 p-4">
                    <h3 class="mb-3 flex items-center gap-2 text-sm font-semibold text-green-800">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($jadwalHariIni as $j)
                            <div class="flex items-center gap-2.5 rounded-lg border border-green-200 bg-white px-3 py-2">
                                <span
                                    class="font-mono text-xs font-semibold text-green-700">{{ substr((string) $j->jam_mulai, 0, 5) }}</span>
                                <span class="h-3 w-px bg-green-200"></span>
                                <span class="text-xs font-medium text-gray-700">{{ $j->mapel?->nama ?? '–' }}</span>
                                <span
                                    class="rounded bg-green-100 px-1.5 py-0.5 text-xs text-green-700">{{ $j->kelas?->nama_kelas ?? '–' }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'kelas' }">
                ...
                <div x-show="activeTab === 'kelas'" x-cloak>
                    ...
                    <div x-show="!selectedKelas">
                        @forelse ($kelasDiajar as $kelas)
                            @php
                                ...
                                // Jadwal for this kelas (from all jadwals collection)
                                $jadwalKelas = $jadwals
                                    ->getCollection()
                            ...
                            <div @click="selectedKelas = {{ $kelas->id }}"
                                class="mb-4 overflow-hidden rounded-xl border border-gray-200 bg-white hover:border-green-300 hover:shadow-sm transition-all cursor-pointer group">
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-2.5">
                                        <div
                                            class="flex h-8 w-8 items-center justify-center rounded-full bg-green-100 text-green-700 font-bold text-xs shrink-0">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="flex flex-wrap gap-1.5">
                                            @foreach ($mapelNames as $mn)
                                                <span
                                                    class="rounded-full bg-blue-50 border border-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-700">{{ $mn }}</span>
                                            @endforeach
                                        </div>
                                        <svg class="h-4 w-4 text-gray-400 group-hover:text-green-600 transition-colors shrink-0 ml-1"
                                ...
                                <div class="px-5 py-4">
                                    ...
                                    @else
                                        ...
                                        @endif
                                        <p class="mt-3 flex items-center gap-1 text-xs font-medium text-green-700">
                    ...
                    @foreach ($kelasDiajar as $kelas)
                        @php
                            $kelasAssignments = $assignments->where('kelas_id', $kelas->id);
                            $mapelNames = $kelasAssignments->pluck('mapel.nama')->filter()->unique()->sort()->values();
                            $semesterNames = $kelasAssignments
                                ->pluck('semester.nama')
                                ->filter()
                                ->unique()
                        ...
                        <div x-show="selectedKelas === {{ $kelas->id }}" x-cloak>
                            ...
                            <div class="mb-6 flex items-center gap-3">
                                <div
                                    class="flex h-11 w-11 items-center justify-center rounded-full bg-green-100 text-green-700 font-bold text-sm shrink-0">
                                    {{ strtoupper(substr($kelas->nama_kelas, 0, 2)) }}
                                </div>
                                <div>
                                    ...
                                    <div class="mt-1 flex flex-wrap items-center gap-1.5">
                                        @foreach ($mapelNames as $mn)
                                            <span
                                                class="rounded-full bg-blue-50 border border-blue-100 px-2 py-0.5 text-xs text-blue-700">{{ $mn }}</span>
                                        @endforeach
                                    ...
                                    <div class="p-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
                                        @foreach ($siswas as $i => $siswa)
                                            <div
                                                ...
                                                <span
                                                    class="w-5 text-right text-xs text-gray-300 font-mono shrink-0">{{ $i + 1 }}</span>
                                                <div
                                    ...
                                    @foreach ($hariOrder as $hari)
                                        @if (isset($jadwalKelas[$hari]))
                                            <div class="border-t border-gray-100 first:border-t-0">
                                                ...
                                                {{ $hari === $todayHari ? 'bg-green-50' : 'bg-gray-50' }}">
                                                    <span
                                                        class="text-xs font-semibold uppercase tracking-wide
                                                    {{ $hari === $todayHari ? 'text-green-700' : 'text-gray-500' }}">
                                                        {{ $hari }}
                                                    </span>
                                                    @if ($hari === $todayHari)
                                                        <span
                                                            class="rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700">Hari
                                                            Ini</span>
                                                    @endif
                                                </div>
                                                ...
                                                @foreach ($jadwalKelas[$hari]->sortBy('jam_mulai') as $jadwal)
                                                    ...
                                                    {{ !$jadwal->is_active ? 'opacity-50' : '' }}">
                                                        ...
                                                        <div class="flex items-center gap-2.5 shrink-0 sm:w-32">
                                                            ...
                                                            {{ $hari === $todayHari ? 'bg-green-100' : 'bg-gray-100' }} shrink-0">
                                                                <svg class="h-4 w-4 {{ $hari === $todayHari ? 'text-green-600' : 'text-gray-400' }}"
                ...
                <div x-show="activeTab === 'jadwal'" x-cloak>
                    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                        @forelse ($jadwalPerHari as $hari => $items)
                            <div class="border-t border-gray-100 first:border-t-0">
                                <div
                                    class="flex items-center gap-2 px-5 py-2.5 {{ $hari === $todayHari ? 'bg-green-50' : 'bg-gray-50' }}">
                                    <span
                                        class="text-xs font-semibold uppercase tracking-wide {{ $hari === $todayHari ? 'text-green-700' : 'text-gray-500' }}">
                                        {{ $hari }}
                                    </span>
                                    <span
                                        class="rounded-full {{ $hari === $todayHari ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500' }} px-2 py-0.5 text-xs font-medium">
                                        {{ $items->count() }} sesi
                                    </span>
                                    @if ($hari === $todayHari)
                                        <span
                                    ...
                                    @foreach ($items as $jadwal)
                                        ...
                                        sm:flex-row sm:items-center sm:gap-4 {{ !$jadwal->is_active ? 'opacity-50' : '' }}">
                                            <div class="flex items-center gap-2.5 shrink-0 sm:w-28">
                                                <div
                                                    class="flex h-8 w-8 items-center justify-center rounded-lg {{ $hari === $todayHari ? 'bg-green-100' : 'bg-gray-100' }} shrink-0">
                                                    <svg class="h-4 w-4 {{ $hari === $todayHari ? 'text-green-600' : 'text-gray-400' }}"
                                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                        ...
                        @empty
                            <div class="p-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-400">Jadwal akan muncul setelah admin menambahkan data.
                                </p>
                            </div>
                        @endforelse
                        ...
                        <table class="w-full text-sm">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th class="px-5 py-3.5 text-left">Kelas Diajar</th>
                                    <th class="px-5 py-3.5 text-left">Semester</th>
                                    <th class="px-5 py-3.5 text-right w-24">Total Kelas</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($mapels as $mapel)
                                    @php
                                        $ma = $assignments->where('mapel_id', $mapel->id);
                                        $kn = $ma->pluck('kelas.nama_kelas')->filter()->unique()->sort()->values();
                                        $sn = $ma->pluck('semester.nama')->filter()->unique()->sort()->values();
                                    @endphp
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-2.5">
                                                <div
                                                    class="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-100 text-indigo-700 text-xs font-bold shrink-0">
                                        ...
                                        <td class="px-5 py-4">
                                            <div class="flex flex-wrap gap-1.5">
                                                @forelse ($kn as $k)
                                                    <span
                                                        class="rounded-full bg-green-100 px-2.5 py-1 text-xs font-medium text-green-800">{{ $k }}</span>
                                                @empty
                                        ...
                                        <td class="px-5 py-4">
                                            <div class="flex flex-wrap gap-1.5">
                                                @forelse ($sn as $s)
                                                    <span
                                                        class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600">{{ $s }}</span>
                                                @empty
                                                    <span class="text-gray-400">–</span>
                                                @endforelse
                                            </div>
                                        ...
                                        <td class="px-5 py-4 text-right">
                                            <span
                                                class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 text-sm font-bold text-gray-700">
                                                {{ $kn->count() }}
                                            </span>
                                        </td>
                                    </tr>
