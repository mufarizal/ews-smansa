...
<div class="mx-auto max-w-2xl">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Tambah Tugas Baru</h2>

        <form method="POST" action="{{ route('guru_mapel.tugas.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center gap-2">
                        <input type="radio" id="jenis_online" name="jenis" value="online" {{ old('jenis') == 'online' ? 'checked' : '' }} required
                            class="h-4 w-4 border-gray-300 text-green-600 focus:ring-green-500">
                        <span class="text-sm">Online (dengan soal)</span>
                    </label>
                    <label class="inline-flex items-center gap-2">
                        <input type="radio" id="jenis_offline" name="jenis" value="offline" {{ old('jenis') == 'offline' ? 'checked' : '' }} required
                            class="h-4 w-4 border-gray-300 text-green-600 focus:ring-green-500">
            ...
            <div id="link-meeting-group" class="hidden">
                <label for="link_meeting" class="mb-1.5 block text-sm font-medium text-gray-700">
                    Link Meeting (Google Meet/Zoom)
                </label>
                <input type="url" id="link_meeting" name="link_meeting" value="{{ old('link_meeting') }}"
                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="date" id="tanggal_tugas" name="tanggal_tugas" value="{{ old('tanggal_tugas') }}" required
                        class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100">
                    @error('tanggal_tugas')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_deadline" name="tanggal_deadline" value="{{ old('tanggal_deadline') }}"
                        class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-green-600 focus:ring-2 focus:ring-green-100">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const materiData = {};
    @foreach ($guruMapelKelas as $gmpk)
        materiData[{{ $gmpk->id }}] = @json($gmpk->babs->flatMap(fn($bab) => $bab->materi)->map(fn($m) => ['id' => $m->id, 'judul' => $m->judul]));
    @endforeach

    function updateMateri() {
        const selectedId = mapelSelect.value;
        materiSelect.innerHTML = '<option value="">-- Pilih Materi --</option>';
        if (!selectedId || !materiData[selectedId]) {
            materiSelect.disabled = true;
            return;
        }
        materiData[selectedId].forEach(m => {
            const opt = document.createElement('option');
            opt.value = m.id;
            opt.textContent = m.judul;
            if (m.id == {{ old('materi_id') ?? 'null' }}) opt.selected = true;
    ...
    // Init
    if (mapelSelect.value) updateMateri();
    toggleLinkMeeting();
});
</script>
@endsection