...
@section('content')
<div class="mx-auto max-w-7xl" x-data="{ tab: 'daftar-soal' }">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Tugas Siswa</p>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('guru_mapel.tugas.edit', $tugas) }}"
            ...
            <a href="{{ route('guru_mapel.tugas.nilai.index', $tugas) }}"
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.7fr)] lg:items-start">
        <section class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="rounded-xl bg-slate-50 px-4 py-3">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $tugas->guruMapelKelas->kelas->nama_kelas ?? '-' }}</p>
                    </div>
                </div>
                ...
                @if ($tugas->materi)
                    <div class="mt-4 rounded-xl bg-slate-50 px-4 py-3">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">
                            {{ $tugas->materi->bab->nama_bab ?? '-' }}: {{ $tugas->materi->judul ?? '-' }}
                        </p>
                    </div>
                @endif
                ...
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div class="rounded-xl bg-slate-50 px-4 py-3">
                        ...
                        <span class="mt-1 inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $tugas->jenis === 'online' ? 'bg-blue-100 text-blue-700' : 'bg-stone-100 text-stone-700' }}">
                            {{ ucfirst($tugas->jenis ?? 'Offline') }}
                        </span>
                    </div>
                    @if ($tugas->jenis === 'online' && $tugas->link_meeting)
                        <div class="rounded-xl bg-slate-50 px-4 py-3">
                ...
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div class="rounded-xl bg-slate-50 px-4 py-3">
                        <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Tanggal Tugas</p>
            ...
            @if ($tugas->jenis === 'online')
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="border-b border-gray-200 mb-4">
                        <nav class="-mb-px flex space-x-6">
                            <button type="button" @click="tab = 'daftar-soal'"
                                :class="tab === 'daftar-soal' ? 'border-green-600 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                                class="whitespace-nowrap border-b-2 py-2 text-sm font-medium transition-colors">
                                Daftar Soal
                            </button>
                            <button type="button" @click="tab = 'tambah-soal'"
                                :class="tab === 'tambah-soal' ? 'border-green-600 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                                class="whitespace-nowrap border-b-2 py-2 text-sm font-medium transition-colors">
                                Tambah Soal
                            </button>
                        </nav>
                    ...
                    <div x-show="tab === 'daftar-soal'">
                        @if ($tugas->soalTugas->isEmpty())
                            <div class="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center">
                                ...
                                <p class="text-sm text-gray-500">Belum ada soal. Tambahkan soal untuk tugas ini.</p>
                            </div>
                        @else
                            <div class="space-y-4">
                                @foreach ($tugas->soalTugas as $index => $soal)
                                    <article class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                        <div class="flex items-start justify-between gap-3">
                                            ...
                                            <div class="flex shrink-0 gap-2">
                                                <a href="{{ route('guru_mapel.tugas.soal.edit', [$tugas, $soal]) }}"
                                                    class="inline-flex items-center justify-center h-7 w-7 rounded-md bg-white text-blue-600 hover:bg-blue-50">
                                                    <i class="ti ti-edit text-sm"></i>
                                                </a>
                                                <form method="POST" action="{{ route('guru_mapel.tugas.soal.destroy', [$tugas, $soal]) }}"
                    ...
                    <div x-show="tab === 'tambah-soal'" x-cloak>
                        <form method="POST" action="{{ route('guru_mapel.tugas.soal.store', $tugas) }}" class="space-y-4">
                            @csrf
        ...
        <aside class="space-y-4 lg:sticky lg:top-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-semibold text-slate-900 mb-3">Statistik Nilai</h2>
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Total Siswa</span>
                        <span class="font-semibold text-slate-900">{{ $tugas->nilaiTugas->count() }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Sudah Dinilai</span>
                        <span class="font-semibold text-emerald-700">{{ $tugas->nilaiTugas->whereNotNull('nilai')->count() }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Belum Dinilai</span>
                        <span class="font-semibold text-amber-700">{{ $tugas->nilaiTugas->whereNull('nilai')->count() }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Terlambat</span>
                        <span class="font-semibold text-rose-700">{{ $tugas->nilaiTugas->where('is_late', true)->count() }}</span>
                    </div>
                </div>
            </div>
        </aside>
